...
final class AirplayServerManager {
    static let sharedManager = AirplayServerManager()
    
    var airplayServers : [AirplayServer] = []
    
    func beginManagingServer(server : AirplayServer) {
        self.airplayServers.append(server)
    }
    
    func stopManagingServer(server : AirplayServer) {
        self.airplayServers.removeAtIndex(self.airplayServers.indexOf(server)!)
    }
}

final class AirplayServer: NSObject, GCDAsyncSocketDelegate, GCDAsyncUdpSocketDelegate {
    ...
    private var remote = AirplayRemote()
    ...
    private var serverName = "Aeroplay Server"
   
    private var UID : UInt8 = 0
    
    var serverPortNumber = 6010 // ===============
    var controlPortNumber = 6011 // Default values. Must be different if running multiple servers.
    var timingPortNumber = 6012 // ===============
    
    var track = Track()

    // MARK: Main Functions
   
    init(serverName : String?, UID : UInt8, serverPortNumber: Int, controlPortNumber: Int, timingPortNumber: Int) {
        super.init()
        ...
        self.controlPortNumber = controlPortNumber
        self.timingPortNumber = timingPortNumber
        
        AirplayServerManager.sharedManager.beginManagingServer(self)
    }
    
    deinit {
        AirplayServerManager.sharedManager.stopManagingServer(self)
    }

    func start() {
        
        let lower : UInt32 = 1000
        let upper : UInt32 = 5000
        let randomNumber = arc4random_uniform(upper - lower) + lower
        let portNumber = randomNumber
        ...
        do {
            ...
            try socket.acceptOnPort(UInt16(portNumber))
        } catch { print("error") }

        let name = getMacAddress().reduce("", combine: {$0 + String(format: "%02X", $1)}) + "@\(self.serverName)"
        print(name)
        service = NSNetService(domain: "", type: "_raop._tcp.", name: name, port: Int32(portNumber))
    ...
    private func parseRtspData(data: NSData, fromSocket sock: GCDAsyncSocket!) {
        ...
        case "SETUP":
            response += ["Transport: RTP/AVP/UDP;server_port=\(self.serverPortNumber);control_port=\(self.controlPortNumber)"]
            response += ["Session: 1"]
    ...
    private func respondToChallenge(challenge: String, fromSocket sock: GCDAsyncSocket!) -> String {
        ...
        print(address.subdataWithRange(range))
    ...
    private func createSessionSockets() {
        ...
        do {
            try serverPort.bindToPort(UInt16(self.serverPortNumber))
            try controlPort.bindToPort(UInt16(self.controlPortNumber))
            try timingPort.bindToPort(UInt16(self.timingPortNumber))
    ...
    private func getMacAddress() -> [UInt8] {
        let macAddress = [UInt8(10),UInt8(10),UInt8(10),UInt8(10),UInt8(10),UInt8(self.UID)]
        return macAddress
    }
    ...
    func udpSocket(sock: GCDAsyncUdpSocket!, didReceiveData data: NSData!, fromAddress address: NSData!,
        ...
        if sock.localPort() == UInt16(self.serverPortNumber) {
            dispatch_async(processQueue, {self.processPacketType(data)})
        }
       
        // Control port
        if sock.localPort() == UInt16(self.controlPortNumber) {
...
final class AirplayRemote: NSObject, NSNetServiceDelegate, NSNetServiceBrowserDelegate, GCDAsyncSocketDelegate {
